...
class MathQuest(tk.Tk):
    ...
    def create_welcome_screen(root):
        ...
        root.time_limit_button_image = PhotoImage(
            file=r"Icons/button_time-limit-game.png"
        )
        ...
        root.start_time_limit_button.place(
            x=start_time_limit_game_x, y=start_time_limit_game_y
        )

        root.score_limit_button_image = PhotoImage(
            file=r"Icons/button_score-limit-game.png"
        )
        root.start_score_limit_button = tk.Button(
            root,
            command=root.start_score_limit_game,
            image=root.score_limit_button_image,
        )
        start_score_limit_game_x = 550
        start_score_limit_game_y = 340
        root.start_score_limit_button.place(
            x=start_score_limit_game_x, y=start_score_limit_game_y
        )

        # Create a button widget to exit full screen mode
        root.original_screen_size = PhotoImage(
            file=r"Icons/button_original-screen-size.png"
        )
    ...
    def start_time_limit_game(root):
        ...
        root.get_ready_label = tk.Label(
            root, text="Get Ready!!", font=("Tahoma", 40), background="#558fe6"
        )
    ...
    def time_limit_countdown(root):
        ...
        root.time_limit_countdown_lbl = tk.Label(
            root, text=time_limit_countdown, font=("Tahoma", 30), background="#558fe6"
        )
