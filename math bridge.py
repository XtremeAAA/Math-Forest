...
class MathBridge(tk.Tk):
    ...
    def start_score_limit_game(root):
        ...
        root.score_game_banner = PhotoImage(file=r"Icons2/Score Limit Banner.png")
        root.score_game_banner_lbl = tk.Label(root, image=root.score_game_banner)
        root.score_game_banner_lbl.pack(pady=30)
        ...
        home_y = 700
        root.home_button.place(x=home_x, y=home_y)
        root.score_game_question()

    def score_game_question(root):
        # generates the integers for my questions
        coefficient = random.randint(1, 10)
        constant_1 = random.randint(5, 20)
        constant_2 = random.randint(40, 80)
        ...
        # compiles the generated integers into a question
        question_text = f"{coefficient}x + {constant_1} = {constant_2}. What is x?"
        root.score_game_display_question()
        root.sg_user_input()
        root.sg_submit_answer_button()

    def score_game_display_question(root):
            root.question_text_lbl = tk.Label(
                root, text=question_text, font=("Tahoma", 40), background="#142948"
            )
            root.question_text_lbl.pack(pady=(180, 0))

    def sg_validate_input(tl_user_input):
            """Validate the input string to only allow numbers and a single full stop."""
            if sg_user_input.count(".") != 1:
                return False
            
            parts = sg_user_input.split(".")
            for part in parts:
                if not part.isdigit():
                    return False
    ...
    def sg_user_input(root):
        global sg_user_input
        """Create an Entry widget with validation to only allow numbers and a single full stop."""

        sg_user_input = tk.Entry(
            root,
            width=60,
            justify="center",
        )
        sg_user_input.pack()
        sg_user_input.focus_set()  # Set the focus to the Entry widget
        root.x2dp_test = tk.Label(
            root,
            text=x2dp,
            font=("Tahoma", 30),
        ...
        root.x2dp_test.place(x=70, y=160)
        root.bind("<Return>", lambda event: root.sg_check_answer())

    def sg_submit_answer_button(root):
        root.sg_submit_answer_png = PhotoImage(file=r"Icons2/button_submit-answer.png")
        root.sg_submit_button = tk.Button(
            root,
            command=root.tl_check_answer,
            image=root.sg_submit_answer_png,
            background="#142948",
        )
        root.sg_submit_button.place(x=555, y=450)

    def sg_check_answer(root, event=None):
        global sg_user_input
        global x2dp
        global sg_score

        user_answer = sg_user_input.get()  # Get the user's input

        if not root.validate_input(user_answer):
            messagebox.showerror("Invalid Input", "Please enter a valid number with two decimal places.")
            sg_user_input.delete(0, 'end')  # Clear the input field
            sg_user_input.focus_set()
            return
        ...
        if round(user_answer, 2) == x2dp:  # Compare the rounded user's answer to x2dp
            sg_score += 1
            root.sg_score_lbl.config(text=f"Score = {sg_score}")  # Update the score label
            sg_user_input.pack_forget()
            root.question_text_lbl.pack_forget()
            root.score_game_question()  # Generate a new question
        else:
            sg_user_input.pack_forget()
            root.question_text_lbl.pack_forget()
            root.score_game_question()  # Generate a new question

    def sg_score_def(root):
        global sg_score
        root.sg_score_lbl = tk.Label(
            root,
        ...
        root.sg_score_lbl.place(x=70, y=100)
